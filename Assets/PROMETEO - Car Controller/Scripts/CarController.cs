...
public class CarController : ILiftable
{
    ...
    public void Turn(float direction)
    {
        ...
        // Clamp the steering axis value between -1 and 1
        steeringAxis = Mathf.Clamp(steeringAxis, -1f, 1f);

        // Calculate the steering angle based on the steering axis and max angle
        var steeringAngle = steeringAxis * maxSteeringAngle;

        // Apply the steering angle to the front wheels
        if (!isForklift)
        {
            // Regular steering: apply the angle directly
            frontLeftCollider.steerAngle = Mathf.Lerp(frontLeftCollider.steerAngle, steeringAngle, steeringSpeed);
            frontRightCollider.steerAngle = Mathf.Lerp(frontRightCollider.steerAngle, steeringAngle, steeringSpeed);
        }
        else
        {
            // Forklift steering: invert the angle for tighter maneuvering
            frontLeftCollider.steerAngle = Mathf.Lerp(frontLeftCollider.steerAngle, -steeringAngle, steeringSpeed);
            frontRightCollider.steerAngle = Mathf.Lerp(frontRightCollider.steerAngle, -steeringAngle, steeringSpeed);
        }
    }
